...
def get_args():
    ...
    p.add_argument('--kl_mode', type=str, default='iid_node',
                   choices=['iid_node', 'centered'],
                   help='KL regularization mode: iid_node uses model-provided KL; centered uses center-subtracted regularization in the training script.')
    p.add_argument('--kl_center_w', type=float, default=1.0,
                   help='Weight for center alignment term in centered KL mode.')
    p.add_argument('--kl_center_var_w', type=float, default=0.0,
                   help='Optional weight for matching batch-wise variance of center to 1.0 in centered KL mode (requires sufficiently large batch).')
    p.add_argument('--kl_rel_w', type=float, default=0.05,
                   help='Weight for relative (centered) latent L2 regularization in centered KL mode (keep small).')
# --- Optim & sched ---
...
def run_epoch(model, loader, device, std, scaler, *,
    ...
    for batch in tqdm(loader, desc=desc, leave=False):
        ...
        latent_low_dense = None  # (B,Nmax,D) when return_latent=True
        ...
        with autocast(enabled=fp16):
            if isinstance(model, MultiTaskMoE_TS):
                if lam_kl > 0.0:
                    (coords_pred, charge_pred, elem_pred, energy_pred, charge_mask, bal, stop_logit, latent_low_dense, kl_loss) = model(batch, return_latent=True)
                else:
                ...
                if lam_kl > 0.0:
                    (coords_pred, charge_pred, elem_pred, energy_pred, charge_mask, bal, feas_logits, latent_low_dense, kl_loss) = model(batch, return_latent=True)
                else:
            ...
            if lam_kl > 0.0:
                if args.kl_mode == 'iid_node':
                    loss_tot = loss_tot + (lam_kl * kl_loss)
                elif args.kl_mode == 'centered':
                    if latent_low_dense is None:
                        raise RuntimeError("Centered KL mode requires return_latent=True from the model to provide latent_low_dense.")
                    # latent_low_dense: (B,Nmax,D) ; charge_mask: (B,Nmax,1)
                    m = charge_mask.float()
                    denom = m.sum(dim=1).clamp(min=1.0)  # (B,1)
                    center = (latent_low_dense * m).sum(dim=1) / denom  # (B,D)
                    rel = latent_low_dense - center.unsqueeze(1)  # (B,Nmax,D)

                    # 1) Center alignment: encourage mean ~ 0
                    loss_center = (center ** 2).mean()

                    # 2) Optional: encourage batch-wise variance ~ 1
                    if args.kl_center_var_w > 0.0:
                        var = center.var(dim=0, unbiased=False)
                        loss_center_var = ((var - 1.0) ** 2).mean()
                    else:
                        loss_center_var = torch.zeros([], device=center.device)

                    # 3) Relative latent weak regularization (energy / scale control)
                    D = rel.size(-1)
                    loss_rel = (rel.pow(2) * m).sum() / (m.sum() * D).clamp(min=1.0)

                    centered_kl = args.kl_center_w * loss_center + args.kl_center_var_w * loss_center_var + args.kl_rel_w * loss_rel

                    loss_tot = loss_tot + (lam_kl * centered_kl)
                    # overwrite kl_loss for logging
                    kl_loss = centered_kl.detach()
                else:
                    raise ValueError(f"Unknown kl_mode: {args.kl_mode}")
            if isinstance(model, MultiTaskMoE_TS):
